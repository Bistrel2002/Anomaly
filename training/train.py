...
# Fichier pour le preprocessing, l'entraînement et la sauvegarde du modèle
"""
import pandas as pd
import numpy as np
...
# Create an output directory for EDA plots
...
try:
    ...
    print(f"Error: Could not find {data_path}. Please make sure the data is in the correct location.")
    exit()
...
axes[0].set_xlim([min(df['Amount']), max(df['Amount'])]) # Adjust x-axis limits if needed
...
# Let's pick a few features that are often discriminative
...
print("\n" + "-" * 50)
print("6. Data Cleaning, Scaling, and Pre-processing for Modeling")
print("-" * 50)
